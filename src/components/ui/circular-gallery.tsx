...
class Media {
	...
	onResize({
		screen,
		viewport,
	}: {screen?: iScreenSize; viewport?: iViewport} = {}) {
...
class App {
	...
	onResize() {
		...
		if (this.medias) {
			this.medias.forEach((media) => {
				{
					return media.onResize({screen: this.screen, viewport: this.viewport});
				}
			});
		}
	}
	...
	update() {
		...
		if (this.medias) {
			this.medias.forEach((media) => {
				return media.update(this.scroll, direction);
			});
		}
...
interface iCircularGalleryProps {
	items?: {image: string; text: string}[];
	bend?: number;
	textColor?: string;
	borderRadius?: number;
